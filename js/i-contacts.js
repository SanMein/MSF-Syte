document.addEventListener('DOMContentLoaded', () => {
    ...
    tl.to('.navbar', { y: 0, duration: 0.5 })
      ...
      .to('.animate-bounce', { y: 0, opacity: 1, stagger: 0.2, duration: 0.8 }, '-=0.5')
      .to('.terminal-status', { opacity: 1, duration: 0.5 }, '-=0.5');

    // Link highlighting
    const links = document.querySelectorAll('.nav-link');
    const currentPage = window.location.pathname.split('/').pop() || './src/i-contacts.html';
    ...
    if (localStorage.getItem('isLoggedIn') === 'true') {
        ...
        loginLink.setAttribute('href', './src/i-personal-account.html');
    }
});
